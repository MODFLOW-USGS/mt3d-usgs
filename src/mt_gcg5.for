...
C
C--ALLOCATE
      ALLOCATE(MXITER,ITER1,ISOLVE,NCRS,IPRGCG,ACCL,CCLOSE,INOCROSS)
C
...
C
C--ALLOCATE ARRAY AND INITIALIZE
      ALLOCATE(LRCH(3,MXITER*ITER1))
      IF(NCRS.GT.0) THEN
        ALLOCATE(A(NODES*19))
      ELSE
        ALLOCATE(A(NODES*7))
...
C
     ...
     &                         NCRS,IPRGCG,METHOD=>ISOLVE,RELAX=>ACCL
C
      IMPLICIT  NONE
      INTEGER   I,IDELTA,IRDEL,IPN,
     &          IPA,IAPN,IATPN,IWK,IOUT,II,
     &          ITP,LITP,ITO,ICNVG,LICNVG,IPLACE,NTRANS,
     &          KSTP,KPER,ICBUND,KLAYER,IROW,JCOLMN,IJ,
     &          IR,IC
      REAL      CNEW,DELC,DCOLD,ALN,CHANGE,CTEMP,CHTMP,
     &          RLN,HT2,TIME2,DELPN,PNAPN,SCALE,
     &          DELAPN,APNPA,GSTOP,RHSNORM,TINY
      DIMENSION CNEW(NODES),ICBUND(NODES)

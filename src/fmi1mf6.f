...
C
      FILNUM = FILNUM + 1
C
C-------SORT OUT WHICH FILE IS GRB, BUD, AND HDS (DON'T RELY ON FILE EXTENSION, SINCE THESE CAN BE ARBITRARY)
      IF(IU.EQ.0) THEN
...
C      CHARACTER(LEN=:), ALLOCATABLE   :: TEXT
      ...
      INTEGER :: NUM
C
C--   AS OF RELEASE 1.1.1, NO SUPPORT FOR PKG2PKG FLOWS FOR MF6-GENERATED FLOW SOLUTIONS
      ALLOCATE(NSFR2LAK,NSFR2UZF,NLAK2UZF,NSNK2UZF)
      NSFR2LAK=0
      NSFR2UZF=0
      NLAK2UZF=0
      NSNK2UZF=0

...
C--CHECK HOW MANY ELEMENTS OF ARRAYS X AND IX ARE USED
 ...
 1090 FORMAT(1X,I10,' ELEMENTS OF THE  X ARRAY USED BY THE SSM PACKAGE'
     & /1X,I10,' ELEMENTS OF THE IX ARRAY USED BY THE SSM PACKAGE'/)
C
C--NORMAL RETURN
      RETURN
      END
...
C
     ...
     &          FTLK,FLAK,FMNW,FDRT,FETS,FSWT,FSFR,FUZF
      ...
      DIMENSION SS(8,MXSS),SSMC(NCOMP,MXSS),CRCH(NCOL,NROW,NCOMP),
...
C
     ...
     & QSTO,CNEW,ISS,A,RHS,NODES,UPDLHS,MIXELM,RETA,COLD,IALTFM,
     & INCTS,MXWEL,IWCTS)
C ******************************************************************
...
C
     ...
     &          MHOST,KHOST,IHOST,JHOST
      INTEGER   IALTFM,INCTS,MXWEL,IWCTS(MXWEL)
      REAL      CNEW,RECH,CRCH,EVTR,CEVT,SS,SSMC,SSG,
     &          CTMP,QSS,QCTMP,DELR,DELC,DH,QSTO,A,RHS,RETA,COLD
      LOGICAL   UPDLHS,FWEL,FDRN,FRCH,FEVT,FRIV,FGHB,FSTR,FRES,
     &          FFHB,FIBS,FTLK,FLAK,FMNW,FDRT,FETS,FSWT,FSFR,FUZF
      DIMENSION ICBUND(NCOL,NROW,NLAY,NCOMP),SS(8,MXSS),SSG(5,MXSS),
     ...
     &          A(NODES),RHS(NODES),RETA(NODES,NCOMP),
...
C--TRANSIENT FLUID STORAGE TERM
     ...
     1          *RETA(N,ICOMP)*COLD(J,I,K,ICOMP)
                ELSE
...
C
     ...
     & DH,IRCH,RECH,CRCH,IEVT,EVTR,CEVT,MXSS,NTSS,SS,SSMC,SSG,
     & QSTO,CNEW,RETA,DTRANS,ISS,RMASIO,INCTS,MXWEL,IWCTS,COLD,IALTFM)
C ********************************************************************
...
C
     ...
     &          NTSS,NUM,IQ,K,I,J,ISS,IGROUP,MHOST,KHOST,IHOST,JHOST
      INTEGER   INCTS,MXWEL,IWCTS(MXWEL),IALTFM

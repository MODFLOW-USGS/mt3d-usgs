...
C--ALLOCATE
     ...
     &         CNTE(NCOMP,MXCTS),IWEXT(MXEXT,MXCTS),IWINJ(MXEXT,MXCTS),
...
C
C--EXTERNAL SOURCE
        READ(INCTS,'(1000F10.0)') QINCTS(ICTS),
...
C--READ NOT-TO-EXCEED-CONC
        IF(IFORCE.EQ.0) THEN
          READ(INCTS,'(1000F10.0)') (CNTE(ICOMP,ICTS),ICOMP=1,NCOMP)
...
C--READ INJECTION WELLS TO BE TREATED AND THEIR OPTIONS AND CONC/MASS CHANGE
        DO II=1,NINJ(ICTS)
          IF(ITRTINJ(ICTS).EQ.0 .OR. ITRTINJ(ICTS).EQ.1) THEN
            READ(INCTS,'(4I10)') KINJ(II,ICTS),IINJ(II,ICTS),
...
C
C
      SUBROUTINE CTS5BD(KSTP,KPER,ICOMP,DTRANS,NTRANS)
C ********************************************************************

...
C--ALLOCATE
     ...
     &         iUnitTRNOP(MXTRNOP),
     &         INUZT,INCTS,INTSO,INRTR,INLKT,INSFT,INOCROSS)
...
C
        ...
        IF(LINE(ISTART:ISTOP).EQ.'NODRYBUD') IDRYBUD=0
        CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,INUNIT)
        IF(LINE(ISTART:ISTOP).EQ.'MUTDRY') MUTDRY=1 
        IF(LINE(ISTART:ISTOP).EQ.'C2DRY') IC2DRY=1  
        IF(LINE(ISTART:ISTOP).EQ.'MINVOL') MINVOL=1 
...
C.......NEGATIVE UNIT NUMBERS INDICIATE DEACTIVATING WRITING TO UCN FILE
...
C--WRITE STATUS OF DRY1 AND DRY2 FLAGS
      IF(DOMINSAT.OR.DRYON) THEN
        WRITE(IOUT,'(A)')
     1   ' DRYCELL OPTION ACTIVATED TO LET MASS FLOW THROUGH DRY CELLS'
      ELSE
...
C
      ...
      CHARACTER*1000 LINE
      CHARACTER*20, ALLOCATABLE :: KEYWORDS(:)
      INTEGER IN,OUT,NKEYWORDS,LLOC,ISTART,ISTOP,I,IKEY
      REAL R
C

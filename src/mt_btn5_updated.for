...
C--ALLOCATE SCALAR VARIABLES
      ...
      ALLOCATE(IATS)     
      !ALLOCATE(DRYON)   
      ALLOCATE(MUTDRY,IC2DRY,MINVOL,IDRYBUD,ICTSPKG)
C
...
C  KEYWORDS MAY BE ENTERED IN ANY ORDER.
      ...
      IF(LINE(ISTART:ISTOP).EQ.'MODFLOWSTYLEARRAYS') THEN
        CALL SetMODFLOWStyleArrays
        LLOCSAVE=LLOC
      ELSEIF(LINE(ISTART:ISTOP).EQ.'DRY1') THEN
        DOMINSAT=.TRUE.
        LLOCSAVE=LLOC
...
C--CHECK FOR SECOND KEYWORD
      IF(MODFLOWStyleArrays.OR.DOMINSAT.OR.DRYON) THEN
        LLOC=LLOCSAVE          
        CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,I,R,IOUT,IN)
        IF(LINE(ISTART:ISTOP).EQ.'MODFLOWSTYLEARRAYS') THEN
          CALL SetMODFLOWStyleArrays
...
C--IGNORE TRANSPORT OPTIONS INPUT WHICH ARE DEFINED THROUGH NameFile     
 ...
 1024 FORMAT(1X)
C
C--WRITE STATUS OF DRY1 AND DRY2 FLAGS
      IF(DOMINSAT.OR.DRYON) THEN
        IF(DOMINSAT) WRITE(IOUT,'(A)')
     1   ' DRY1 OPTION ACTIVATED - TO LET MASS ENTER DRY CELLS'
        IF(DRYON) WRITE(IOUT,'(A)')
     1   ' DRY2 OPTION ACTIVATED - TO LET MASS RE-ENTER FROM DRY CELLS'
      ELSE
        WRITE(IOUT,'(A)')' DRY1 OR DRY2 OPTIONS NOT SET'
      ENDIF
C

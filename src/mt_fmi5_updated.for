...
C
     ...
     &          KKK,III,JJJ,ITEMP,IGROUP,NCOMP,ICNT
      REAL      BUFF,SS,QSS,QSTEMP,QSW
      CHARACTER TEXT*16,FPRT*1,LABEL*16
      DIMENSION BUFF(NCOL,NROW,NLAY),ICBUND(NCOL,NROW,NLAY),SS(8,MXSS)
...
!     1  QPRECSFO(NSTRM),QRUNOFSFO(NSTRM),
     ...
     1  IDSPFLG(NINTOT),IDXNIN(NSTRM+1)) !,QINSFO(NINTOT))
      ...
      ALLOCATE(INFLWNOD(NSTRM))                                   !# NEW
      ALLOCATE(CNEWSF(NSFINIT,NCOMP),COLDSF(NSFINIT,NCOMP),       !# NEW
     1  COLDSF2(NSFINIT,NCOMP),CNEWSFTMP(NSFINIT,NCOMP),          !# NEW
     1  DISPSF(NSFINIT,NCOMP))                                    !# NEW
      ISTRM=0
      IDXNIN=0
C

...
C--PRINT PACKAGE NAME AND VERSION NUMBER
 ...
 1030 FORMAT(1X,'TOB5 (EDM CUSTOM)-- TRANSPORT OBSERVATION PACKAGE,',
     & ' VERSION 5, DECEMBER 2011, INPUT READ FROM UNIT',I3)
C
...
!          write(inConcObs,36)
!     &     cobsnam(n),xx2,yy2,kp,ip,jp,icomp,cwgt,ErrMsg   
        ELSEIF(iOutCobs.eq.0 .and. iErrMsg.lt.0) THEN               !edm
          IF(cwgt.le.0) THEN                                        !edm
            goto 29                                                 !edm
          ELSE                                                      !edm
            edm_cal=log10(cwgt)                                     !edm
          ENDIF                                                     !edm
          IF(cobs.gt.0) THEN                                        !edm
            edm_obs=log10(cobs)                                     !edm
          ELSE                                                      !edm
            edm_obs=111                                             !edm
          ENDIF                                                     !edm
          write(inConcObs,37)                                       !edm
     &     cobsnam(n),xx2,yy2,kp,ip,jp,icomp,edm_cal,edm_obs        !edm
        endif
   29 if(inSaveObs.gt.0) then
   ...
   37 format(1x,a12,1p,2g12.4,3i5,3x,i4,2x,2g20.12)                 !edm

c--calculate statistics
      if(iOutCobs.gt.0.and.nobs_active.gt.1) then
C        CALL MOMENT(temp(1,3),nobs_active,AVE,ADEV,SDEV,VAR,SKEW,CURT)
C        CALL PEARSN(temp(1,1),temp(1,2),nobs_active,R,PROB,Z)
      else
        goto 1000
      endif
...
c
C1000 IF(nobs_current.le.0) write(inConcObs,1080)                   !edm
...
c    
C      WRITE(inFluxObs,2) kper,kstp,ntrans,time2                    !edm
      IF(iOutFlux.gt.0 .and. time1.eq.0) THEN                       !edm
        WRITE(inFluxObs,4) 
    ...
    4 format(1x,'  NO. NAME            TIME   SPECIES      ',
     & 'CALCULATED            OBSERVED        Return_Q'
     &  /1x,45(' '),'(mg/wk)             (kg/wk)',/1x,88('.'))      !edm
...
c--cumulate in GroupData
     ...
     &     icomp,fluxcal,fluxobs,weight,error
        elseif(iOutFlux.gt.0 .and. iErrMsg.gt.0) then               !edm
          write(inFluxObs,32) iGroup,fobsnam(n),timeobs,
     &     icomp,fluxcal,ErrMsg
        elseif(iOutFlux.gt.0 .and. iErrMsg.lt.0) then               !edm
          IF(fluxcal.lt.0) THEN                                     !edm
            edm_cal=log10(fluxcal*-1)                               !edm
            edm_cal=edm_cal*-1                                      !edm
          ELSE                                                      !edm
            edm_cal=log10(fluxcal)                                  !edm
          ENDIF                                                     !edm
          IF(fluxobs.lt.0) THEN                                     !edm
            edm_obs=log10(fluxobs*-1)                               !edm
            edm_obs=edm_obs*-1                                      !edm
          ELSE                                                      !edm
            edm_obs=log10(fluxobs)                                  !edm
          ENDIF                                                     !edm
          write(inFluxObs,30) iGroup,fobsnam(n),timeobs,            !edm
     &     icomp,edm_cal,edm_obs,GroupData(7,n)                     !edm
        elseif(iOutFlux.eq.0) then
   ...
   30 format(1x,i4,2x,a12,1p,g12.4,i4,2x,2g20.11,1g12.4)
...
C        CALL MOMENT(grouptmp(1,3),nobs_active,AVE,ADEV,
C     &              SDEV,VAR,SKEW,CURT)
C        CALL PEARSN(grouptmp(1,1),grouptmp(1,2),
C     &              nobs_active,R,PROB,Z)
      else
        goto 1000
      endif
c
...
c
   ...
   72 FORMAT( 1x,'         PROBABILITY OF UN-CORRELATION = ',G15.7)
c
C1000 IF(nobs_current.le.0) write(inFluxObs,1080)                   !edm
...
C      
      if(n.le.1) then
C        call ustop ('N must be at least 2 in subroutine MOMENT',
C     &   ' used by TOB Package')
      endif        
      s=0.
      do j=1,n
        s=s+data(j)
...
C        call ustop('Bad argument x in subroutine BETAI',
C     &   ' used by TOB Package')
      endif  
...
C     
      ...
      enddo      
C      call ustop('a or b too big, or MAXIT too small',
C     & ' in subroutine BETACF used by TOB Package')
    1 betacf=h
C
      RETURN
      END

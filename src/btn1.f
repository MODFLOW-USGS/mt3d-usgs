...
C
      IF(IFMTRF.NE.0) WRITE(IOUT,1038) IFMTRF
      IF(IFMTRF.EQ.0) WRITE(IOUT,1040)
...
C--READ AND ECHO LOGICAL FLAG CHKMAS
 ...
 1066 FORMAT(1X,'     TIME       TOTAL IN      TOTAL OUT      SOURCES',
     ...
     & '        DISCREPANCY(%)               MOBILE_DSLV   ',
     & 'MOBILE_SORB IMMOBILE_DSLV IMMOBILE_SORB ')
...
C--TO FILE [MT3Dnnn.OBS] IF REQUESTED.
     ...
     &     LOCOBS(2,N),LOCOBS(1,N),ICOMP),N=1,NOBS)
        ENDIF
      ENDIF
 1000 FORMAT(1X,I5,1X,1PG13.5,1X,16(G13.5E3,1X))
 1010 FORMAT(1X,I5,1X,1PG13.5,1X,16(G13.5E3,1X)/(1X,20X,16(G13.5E3,1X)))
C
...
C--TO FILE [MT3Dnnn.MAS] IF REQUESTED
     ...
     &        TMASS(1,2,ICOMP),TMASS(2,2,ICOMP), !mobile dissolved, mobile sorbed
     &        TMASS(3,2,ICOMP),TMASS(4,2,ICOMP) !immobile dissolved, immobile sorbed
      ENDIF
 1012 FORMAT(1X,1P,7(G13.5,1X),4X,G10.3,5X,G10.3,4x,4(1X,G13.5))

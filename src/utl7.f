C
C--SUPPORTS, FOR NOW, MT3DMS STYLE ARRAYS
C--TO ACTIVATE, A KEYWORD COULD BE ADDED TO THE BTN FILE,
C--FOR EXAMPLE, "MODFLOWSTYLEARRAYS", WHICH WOULD INITIATE A CALL
C--TO THE SUBROUTINE SETDMOFLOWSTYLEARRAYS AND SET IT TO TRUE.
      MODULE MT3DUTIL
        LOGICAL :: MODFLOWStyleArrays=.FALSE.
      CONTAINS
          SUBROUTINE SetMODFLOWStyleArrays()
            MODFLOWStyleArrays=.TRUE.
          END SUBROUTINE
      END MODULE
C
      ...
      USE MT3DUTIL
      IMPLICIT  NONE
      INTEGER   IA,II,JJ,K,IN,IOUT
      CHARACTER ANAME*24
      DIMENSION IA(JJ,II)
      IF(.NOT. MODFLOWStyleArrays) THEN
C          WRITE(IOUT,50)
          CALL IARRAY_DEPR(IA,ANAME,II,JJ,K,IN,IOUT)
      ELSE
          CALL U2DINT(IA,ANAME,II,JJ,K,IN,IOUT)
      ENDIF
50    FORMAT(1X,'WARNING: MT3DMS STYLE ARRAYS MAY NOT BE SUPPORTED',
     &' IN THE FUTURE.  CONSIDER USING MODFLOW FREE FORMAT CONTROL',
     &' LINES INSTEAD.')
      RETURN
      END
C
C
      SUBROUTINE IARRAY_DEPR(IA,ANAME,II,JJ,K,IN,IOUT)
C ************************************************************
...
C
      ...
      IF(.NOT. MODFLOWStyleArrays) THEN
C          WRITE(IOUT,50)
          CALL RARRAY_DEPR(A,ANAME,II,JJ,K,IN,IOUT)
      ELSE
          CALL U2DREL(A,ANAME,II,JJ,K,IN,IOUT)
      ENDIF
50    FORMAT(1X,'WARNING: MT3DMS STYLE ARRAYS MAY NOT BE SUPPORTED',
     &' IN THE FUTURE.  CONSIDER USING MODFLOW FREE FORMAT CONTROL',
     &' LINES INSTEAD.')
      ...
      END
C
      SUBROUTINE RARRAY_DEPR(A,ANAME,II,JJ,K,IN,IOUT)
C ********************************************************
